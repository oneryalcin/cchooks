...
> - **`to_json` PreToolUse format: DONE.** `to_json(hook_event_name)` is now event-aware. For `PreToolUse` it emits `hookSpecificOutput.permissionDecision` (`deny`/`block`→`"deny"`; `approve`+`updatedInput`→`"allow"` to auto-approve a modified input; a bare `approve` stays empty = normal flow, preserving "no opinion"); other events keep the top-level `decision`/`reason` form, which is still current for them. Threaded through both transports (`write_stdout` and `_asgi_app` pass the event name). Existing tests that asserted the deprecated PreToolUse output were updated to the new shape. 637/637 pass.
